...
tags: ['JavaScript']
date: '2019-10-28'
keywords: ['JavaScript', 'ES6', 'var', 'let', 'const']
---

## 過去常用的 var 與 window

以前寫 JavaScript 總是用 var 來宣告變數，但是使用 var 宣告的變數，會汙染全域環境，例如：
...
我們透過 var 使用於 for 迴圈時常見的問題，來進一步解釋 let 的特性。
...
這是因為 `i` 的值已經被「全域變數」所影響。當 For 迴圈跑完之後，`i` 的值已經等於 `i++` 四次之後的結果，且汙染了全域變數。

### 使用 let 改善

let 的功能是 For 迴圈執行每一次，裡面的 `i` 都可以存活在個別的「大括號作用域」裡面，所以彼此執行的內容不像 var 一樣會被干擾。
...
1. 執行 `let i = 0`，此時的 i 存活在等於 0 的 { } 作用域裡面，執行 `function (){ alert(i+1)}`，變成 `alert(1)`
2. 執行 `let i = 1`，此時的 i 存活在等於 1 的 { } 作用域裡面，執行 `function (){ alert(i+1)}`，變成 `alert(2)`

## ES6 const 的特性：唯獨

const 是唯讀變數（即不能修改），常用在一些不能被變更的變數，例如：網址、圓周率。

不過有個例外，就是 { 物件 } 跟 [ 陣列 ] 還是會被變更，這方面可以用 `freeze()` 方法解決。

### Object.freeze()

`freeze()` 的功能顧名思義就是「凍結」一個物件，用於防止物件新增屬性，或是防止原有的屬性被刪除。
...
A：因為 JavaScript 編譯時，會預設讓用 var 宣告的變數向上提升，只要是建立變數或 Function 都會自動提升到最上面去。
...
此時，很多人會認為 var 有向上提升的特性，而 let 與 const 沒有，但這是錯誤的喔。

其實 let 與 const 也有 Hoisting，但是它們不會先初始化為 undefined，而是會形成 TDZ，也就是「暫時性死區」。  
在「提升之後」至「賦值之前」這段期間，如果在賦值前試圖取值，就會拋出錯誤。
...
這邊就回到我們一開始所說的，使用 let 與 const 能避免掉使用 var 會汙染全域變數的副作用。
