...
> 另一種建立方法 `createRoutesFromElement` 較為冗長，跟以前使用 JSX 的寫法相似，這裡不予介紹。
...
此外，在 `<NavLink>` 使用 `end` 屬性，可以確保該元件不會在進入子路由時，被匹配為 `isActive` 等狀態。
...
以下的 RootLayout 元件是一個父路由，我們會使用 `children` 配置子路由 (下面會介紹到)，而那些子頁面的內容就會顯示在 `<Outlet />` 這個輸出點的位置。
...
接下來，還有一個小地方可以調整，可以看到子路由的 HomePage 與父路由其實是指向同一個路由，這種情況下，我們可以把 `path: ""` 改用 `index` 這個特殊的屬性去定義，明顯表示 HomePage 是一個 Index Route。
...
看完這篇文章，我們認識了 React Router V6 的基礎架構，瞭解如何用它來建立一套路由系統，並且能實際應用在專案上。

下一篇文章會介紹 React Router V6 提供的全新功能 `Loader`，讓我們的路由系統更加全面。
