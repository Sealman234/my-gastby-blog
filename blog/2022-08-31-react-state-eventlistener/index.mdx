...
這種情況其實也很常見，例如使用 UI 套件像是 Ant Design 的時候，如果想監聽 Ant Design Table 的滾動，會發現 Table Body 的位置被包裝在元件渲染後的內層，我們沒辦法直接透過 JSX 掛上 `onScroll` 事件。
...
這麼做之後，你會發現 Scroll 觸發時，所設定的 State 沒有被正確更新成 `newColumns`，然而你在 React DevTools 裡面卻可以看到 State 已經更新，只是畫面上 Table 所使用的 State 沒有更新。
...
我們把更新的 State 放到 useEffect 的 Dependency 陣列中，這樣當 State 改變時，就會重新為 DOM 元素綁定事件，此時的 Listener 所使用的值就會是更新後的 State 囉！
